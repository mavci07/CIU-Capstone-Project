...
class MyCustomFormState extends State<Siparisduzenle> {
  ...
  void initState() {
    ...
    grupduzenle()
        .whenComplete(() => listeal().whenComplete(() => yuklendi = 1));
    // _dbstoklar.grupliste().then((gelen) {
    //   setState(() {
    //     gelen.forEach((veri) {
    //       gruplar.add(veri);
    //     });
  ...
  Future<void> listeal() async {
    ...
    if (list.isNotEmpty) {
      setState(() {
        list.forEach((note) {
          stokh.add(StokHmodel.fromJson(note));
        });
        cariara.text = stokh[0].cariunvan;
        _carikod = stokh[0].carikod;
        tutaral();
        _aciklama.text = stokh[0].aciklama;
        _sipno = stokh[0].sipno;
  ...
  void carikodguncelle() async {
    ...
    var durum = await _dbStokhareket.cariguncelle(
        _sipno, _carikod, cariara.text, _aciklama.text);
  }
              ...
              actions: <Widget>[
                IconButton(
                    ...
                    onPressed: () => Navigator.of(context).push(
                        MaterialPageRoute(
                            builder: (context) => Yenitahsilat(_carikod))))
              ],
              centerTitle: true,
              title: Column(
                ...
                children: <Widget>[
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintText: "Enter Stock Code and Barcode",
                          ...
                          if (_miktar.text.isEmpty ||
                            ...
                            showDialog<void>(
                              ...
                              builder: (BuildContext context) {
                                return AlertDialog(
                                  title: const Text('ERror !'),
                                  content: Text('QUANTITY and PRICE Invalid !'),
                                  actions: <Widget>[
                                    FlatButton(
                                      onPressed: () {
                                        Navigator.pop(context);
