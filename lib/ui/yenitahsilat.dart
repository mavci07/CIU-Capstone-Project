...
import 'package:karakaya_soguk/ui/geneltoplam.dart';
import 'package:uuid/uuid.dart';
import 'dart:core';
import 'dart:convert';
import 'anasayfa.dart';

class Yenitahsilat extends StatefulWidget {
  final String _carikodgelen;
  const Yenitahsilat(this._carikodgelen);
  @override
...
class MyCustomFormState extends State<Yenitahsilat> {
  ...
  void initState() {
    ...
    _kayno = kaynoal();
    if (widget._carikodgelen == "")
      Timer.run(() {
        _carimodal();
      });
    else {
      var gelen =
          carilist(widget._carikodgelen).whenComplete(() => setState(() {
                _carikod = carig[0].carikod.toString();
                cariara.text = carig[0].cariunvan.toString();
                _bakiye = double.parse(carig[0].bakiye.replaceAll(',', '.'));

                _borc = double.parse(carig[0].borc.replaceAll(',', '.'));
                _alacak = double.parse(carig[0].alacak.replaceAll(',', '.'));
              }));
    }
    tutarfocus.addListener(() {
      if (tutarfocus.hasFocus) {
        tutar.selection =
            TextSelection(baseOffset: 0, extentOffset: tutar.text.length);
      }
    });
  }
  ...
  void carikodguncelle() async {
    ...
    var durum = await _dbCarihareket.cariguncelle(
        tahsilatno.text, _kayno, _carikod, _aciklama.text);
  }
    ...
    return new WillPopScope(
        ...
        child: new Scaffold(
          appBar: AppBar(
            ...
            actions: <Widget>[
              if (widget._carikodgelen.isNotEmpty)
                IconButton(
                    icon: Icon(
                      Icons.arrow_back,
                      color: Colors.white,
